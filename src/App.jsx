...
/* Merge de 2026-08-14 — as duas linhas redesenharam esta tela em paralelo.
   Ficou o `Painel` da linha visual (repaginação heráldica, hero animado,
   retomar, preparo do mundo) e ficou o `useDashboardData` da linha do SDD,
   que alimenta o sino e cujas fichas-ao-vivo foram dobradas para dentro do
   "Precisa de você" do Painel. O `Dashboard` da linha do SDD segue no repo
   sem ser montado — quem quiser comparar as duas telas ainda tem as duas. */
import { useDashboardData } from "./features/dashboard";
import Painel from "./components/Painel";
/* Modo demo (src/demo/demoMode.js): usuário falso, sem Firestore. Os dois
   pontos abaixo são os únicos que o App precisa conhecer — os outros dois
   ficam nos hooks (useAuth, useCampaign). */
import { DEMO_ON, PLANOS_DEMO } from "./demo/demoMode";
/* Névoa da repaginação heráldica — a camada de movimento por cima do fundo. */
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (DEMO_ON) { aplicarLiveSheets({}); return; }   // demo: sem Firestore
  ...
  const renderScreen = () => {
    ...
    switch(screen){
      case "dashboard": return <Painel system={activeSystem} onCreateChar={()=>setCreatingChar(true)} characters={characters} onSelectChar={c=>{ setCreatedChar(c); setScreen("sheet"); }} onNav={setScreen} userPlans={userPlans} onShowUpgrade={()=>setScreen("planos")}
      ...
      default: return <Painel system={activeSystem} onCreateChar={()=>setCreatingChar(true)} characters={characters} onNav={setScreen} userPlans={userPlans} onShowUpgrade={()=>setScreen("planos")}
        uid={currentUser?.uid || ""} campaigns={campaigns} liveSheets={liveSheets} dados={dadosPainel}
        onOpenCampaign={abrirCampanha} onCreateCampaign={()=>{ setScreen("party"); setShowCreateCampaign(true); }}/>;
    }
  };
